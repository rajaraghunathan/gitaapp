...
def yt_inject_db():
    csv_folder = os.path.join(BASE_DIR, "static", "csv_files")
    ...
    for file in files:
        ...
        with open(csv_path, newline="", encoding="utf-8") as f:
            ...
            rows = list(reader)
        print(f'Writing YouTube Urls for {file} to database')
        print('=='*30)

        lang = 'ta' if 'ta_' in file else 'en'
        ...
        for index, row in enumerate(rows):
            ...
            dict_language = database_url.get(lang)
            if dict_language:
                if dict_language.get(author):
                    print(f'{author} exists for {chapter_num}-{verse_num} and Skipped for language {lang}')
                else:
                    dict_language.update({author: url})
                    print(f'youtube Url for {author} updated for {chapter_num}-{verse_num} for language {lang}')
            else:
                dict_language = {author: url}
                print(f'youtube Url inserted for {chapter_num}-{verse_num} for language {lang}')
            database_url[lang] = dict_language
        db.commit()
        print('**' * 30)
    return None
